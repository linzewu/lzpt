...
<%
	String auditType1 = request.getParameter("be.auditType");
%>
        $(function(){
            $('#myform').form({
...
<body>
	<form action="baseManager!!saveBaseEntity.action"  id="myform" name="myform" method="post" enctype="multipart/form-data">
		<div id="p" class="easyui-panel" style="padding: 10px; min-width: 700px; min-height: 300px;" title="审计报告  添加">
				...
				<table style="width: 680px" border="0" class="base_table" cellpadding="0"  cellspacing="0">
					<tbody>
						<tr>
							...
							<td class="info"><input name="be.userName" class="easyui-validatebox"  /></td>
							<td class="info_title">审计类型：</td>
							<td class="info">
								<input name="be.auditType" class="easyui-combobox"  required="true" id="auditType"
									 data-options="valueField:'value',textField:'label',data:datacode.auditType" />
							</td>
						</tr>
						<tr>
							<td class="info_title">单位：</td>
							<td class="info" colspan="3">
								<input name="be.unitName" class="easyui-combobox"  data-options="valueField:'value',textField:'label',data:datacode.units"     required="true"   style="width: 300px" /></td>
						</tr>
						
						<tr>
							<td class="info_title">备注说明：</td>
							<td class="info" colspan="3">
								<textarea name="be.remark" required="true" class="easyui-validatebox"  rows="10" cols="70"  id="complainContext"></textarea>
							</td>
						</tr>
						<tr>
...
可上传文件大小：20M</label></pre>
				...
				<div align="center">
					<a href="javascript:$('#myform').submit();" class="easyui-linkbutton" data-options="iconCls:'icon-save'">保存</a> <a
						href="baseManager!auditReport!toPage.action" class="easyui-linkbutton" data-options="iconCls:'icon-back'">返回</a>
				</div>
			</div>
		</div>
	</form>
<script type="text/javascript">
    $(function(){
    	$("#auditType").combobox("setValue","<%=auditType1%>");
    });
