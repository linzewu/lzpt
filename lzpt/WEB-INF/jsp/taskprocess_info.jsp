...
<div id="tt" class="easyui-tabs" > 
	<div title="任务详细">
		...
		<div style="margin:10px 0px 10px 0px;text-align: center;">
			<a href="task!toPage.action?pageName=processList" class="easyui-linkbutton" iconCls="icon-cancel">返回</a>
		</div>
		
	</div>
	
	<div title="执行信息">
		<form action="task!addTaskProcess.action" id="fm" method="post">
			<table style="margin-left: 20px; margin-top: 10px;" cellpadding="5px">
				<tr>
					<th><label for="taskProcess.processTitle">上报标题:</label></th>
					<td><input class="easyui-validatebox" id="title" style="width: 600px;" required="true" missingMessage="请填写上报标题" name="taskProcess.processTitle"
						type="text" value="关于:${task.title }任务执行" /><br /> <label>完成纪检监察工作项目名称，不超过100字</label></td>
				</tr>
				<tr>
					<th><label for="task.context">主要内容:</label></th>
					<td><textarea cols="80" rows="10" id="processContext" name="taskProcess.processContext" ></textarea><br> <label>纪检工作主要内容及说明，不超过500字，如内容较多请选择“附近”栏上传附件</label></td>
				</tr>
				<tr>
					<th><label for="taskFile">附件:</label></th>
					<td>
						<div class="fileInfo" id="taskProcessFileInfo"></div> <input type="file" name="uploadFile" alt="文件上传" id="uploadFile"
...
可上传文件大小：20M</label></pre>
				...
				<tr>
					...
					<td><textarea cols="80" id="remark" rows="5"  name="taskProcess.processRemark"></textarea><br> <label>备注信息，不超出200字</label></td>
				</tr>
			</table>
			<input type="hidden" value="" name="taskProcess.processfilesJson" id="taskFiles" >
			<input type="hidden" value="${task.id }" name="taskProcess.task.id" >
			
			<input type="hidden" value="1" name="taskProcess.processState" >
			<div style="margin:10px 0px 10px 0px;text-align: center;">
				<a href="javaScript:void(0)" onclick="saveProcess()" class="easyui-linkbutton" iconCls="icon-save">上报</a>
				<a href="task!toPage.action?pageName=processList" class="easyui-linkbutton" iconCls="icon-cancel">取消</a>
			</div>
		</form>
	</div>
