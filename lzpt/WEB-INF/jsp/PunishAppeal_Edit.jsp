...
<body type="modify">
		...
		<form id="fm" method="post" action="encourageAppeal!modifyEncourageAppeal.action">
			<table title="修改惩罚诉求" style="margin-left: 20px; margin-top: 10px;" cellpadding="5px">
			    <tr>
				...
				<tr>
					...
					<td><input name="ea.etitle" class="easyui-validatebox" style="width: 600px;" value="${ea.etitle }" required="true" missingMessage="请输入标题" /></td>
				</tr>
				<tr>
					<th><label for="ea.eremark">备注:</label></th>
					<td><input name="ea.eremark" id="remark" value="${ea.eremark }" style="width: 600px;"/> 
					<input style="display: none" name="ea.createUser" value="${ea.createUser }"/>
					<input style="display: none" name="ea.createTime" value="<fmt:formatDate value="${ea.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
					<input style="display: none;" name="ea.id" value="${ea.id }"/>
					<input style="display: none" name="ea.etype" value="${ea.etype }"/>
					</td>
				</tr>
				<tr>
					<th><label for="ea.econtext">内容:</label></th>
					<td><textarea name="ea.econtext" cols="80" rows="10" id="context">${ea.econtext }</textarea></td>
				</tr>				
				
			</table>
			<div style="margin:10px 0px 10px 0px;text-align: center;">
				<a href="#" class="easyui-linkbutton" iconCls="icon-ok" onclick="javascript:saveEncourage1()">保存</a>  
