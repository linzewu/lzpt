        ...
        $(function(){
        	...
        	loadInfo("baseManager!!getBaseEntity.action?be.df=d2", 'Criticism',
    				"${param['be.id']}", "myform",function(data){
    					var  contents = data['be.contents'];
    					var strs=contents.split(",");
    					 $("#contentsBox").combobox("setValues",strs);
    				});
        });
    </script>
</head>
				...
				<table style="width: 680px" border="0" class="base_table" cellpadding="0"  cellspacing="0">
					<tbody>
						...
						<tr>
							...
							<td class="info"  >
								<span name="be.startTime"  class="readField"></span>
								<input name="be.startTime"  type="hidden"  id="startTime"  />
							</td>
							<td class="info_title">播放时间：</td>
							<td class="info" ><input name="be.stays" class="easyui-validatebox" required="true" value="30"  />(分钟)</td>
						</tr>
