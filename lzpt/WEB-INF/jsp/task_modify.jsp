...
<body type="modify">
	<div class="easyui-panel" title="添加工作任务">
		<form method="post" action="task!modifyTask.action?task.id=${task.id }" id="fm" enctype="multipart/form-data">
			<table title="添加用户" style="margin-left: 20px; margin-top: 10px;" cellpadding="5px">
				
				<tr>
					<th><label for="task.title">项目标题:</label></th>
					<td><input class="easyui-validatebox" value="${task.title }" id="title" style="width: 600px;" required="true" missingMessage="请填写项目标题"
						type="text" name="task.title" /><br /> <label>完成纪检监察工作项目名称，不超过100字</label></td>
				</tr>
				
				<tr>
					<th><label for="task.startDate">执行时段:</label></th>
					<td><input class="easyui-datebox" value="${task.startDate }" id="startDate" style="width: 200px;" editable="false" required="true" missingMessage="请填写开始时间"
						type="text" name="task.startDate" />至<input class="easyui-datebox" value="${task.endDate }" editable="false" id="endDate" style="width: 200px;" required="true" missingMessage="请填写结束时间"
				...
				<tr>
					...
					<td><input class="easyui-datebox" value="${task.lastDate}" id="lastDate" style="width: 200px;" editable="false" required="true" missingMessage="请填写结束时间"
						type="text" name="task.lastDate" /><br /> <label>如果超过这个期限，任务将会过期，单位得分为0。</label></td>
				</tr>
				
				<tr>
					<th><label for="task.context">主要内容:</label></th>
					<td><textarea cols="80" rows="10" id="context" name="task.context">${task.context }</textarea><br> <label>纪检工作主要内容及说明，不超过500字，如内容较多请选择“附近”栏上传附件</label></td>
				</tr>
				<tr>
...
可上传文件大小：20M</label></pre>
				...
				<tr>
					...
					<td><textarea cols="80" id="remark" rows="5" name="task.remark">${task.remark }</textarea><br> <label>备注信息，不超出200字</label></td>
				</tr>
			</table>
			...
			<input type="hidden" value="${task.filesToJson }" name="task.filesToJson" id="taskFiles" >
		</form>
	</div>
</body>
</html>